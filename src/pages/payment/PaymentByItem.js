...
import { confirmItemPayment, getOrderDetails } from "../../utils/Services";

const PaymentByItem = (props) => {
  const { match, general, auth } = props;
  const order_id = match.params.id;
  const item_id = match.params.item;
  ...
  useEffect(() => {
    goPageTop();
    setLoading(true);
    getOrderDetails(order_id, order_id, auth.shopAsCustomer.id, auth.isShopAsCustomer).then((response) => {
      setLoading(false);
  ...
  let exactItem;
  if (!loading && order) {
    exactItem = order.order_items.find((item) => item.order_item_number == item_id);
  }
  ...
  const getNeedToPay = () => {
    let returnValue = Number(exactItem.due_payment);
    const curiarBill = exactItem.courier_bill || 0;
    const shippingCharge = exactItem.shipping_charge || 0;
    returnValue += Number(curiarBill) + Number(shippingCharge);
    return returnValue;
  };
  ...
  const paymentConfirm = (e) => {
    ...
    if (process) {
      if (trxId) {
        confirmItemPayment(exactItem.order_item_number, {
                      ...
                      <table className='table table table-cart'>
                        ...
                        <tfoot>
                          ...
                          <tr className='summary-total'>
                            ...
                            <td className='text-right'>{`${currency} ${numberWithCommas(
                              Number(exactItem.courier_bill)
                            )}`}</td>
                          </tr>
                          <tr className='summary-total'>
                            <td colSpan={2} className='text-right'>
                              Shipping charge ({currency}
                              {exactItem.shipping_rate} * {exactItem.actual_weight}KG):
                            </td>
                            <td className='text-right'>
                              {`${currency} ${numberWithCommas(Number(exactItem.shipping_charge))}`}
                            </td>
                          </tr>
                          <tr className='summary-total'>
                            <td colSpan={2} className='text-right'>
...
export default connect(mapStateToProps, { confirmCustomerOrder })(withRouter(PaymentByItem));
