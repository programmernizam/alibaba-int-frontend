...
const ManageQuantity = (props) => {
  ...
  const activeConfiguredQtyChanges = (type = "increment") => {
    ...
    if (Number(newQty) <= Number(maxQuantity)) {
      cartProductQuantityUpdate(
        newQty,
        getUpdatedProductPrice(newQty, bulkPriceQuantity, product, rate, ConfiguredItem),
        cartConfigured,
        product_id,
        existsConfig.Id,
        ShippingCharges,
  ...
  const inputQtyChanges = (qty) => {
    if (Number(qty) <= Number(maxQuantity)) {
      cartProductQuantityUpdate(
        qty,
        getUpdatedProductPrice(qty, bulkPriceQuantity, product, rate, ConfiguredItem),
        cartConfigured,
        product_id,
        existsConfig.Id,
        ShippingCharges,
  ...
  const addConfigItemToCart = (qty = 1) => {
    ...
    if (_.isEmpty(activeProduct)) {
      activeProduct = {
        ...
        Quantity: qty,
        Price: getUpdatedProductPrice(qty, bulkPriceQuantity, product, rate, ConfiguredItem),
        hasConfigurators: true,
        IsCart: true,
        isChecked: true,
        ConfiguredItems: [],
        chinaLocalDelivery: 0,
        itemTotal: 0,
      };
    }
