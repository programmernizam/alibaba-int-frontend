...
const RadyDeliverdOrdersList = (props) => {
  const { auth } = props;
  ...
  const pageCount = Math.ceil(readyForDelivery.length / itemsPerPage);

  // Invoke when user click to request another page.
  const handlePaginationClick = (event) => {
    const newOffset = (event.selected * itemsPerPage) % readyForDelivery.length;
  ...
  const allFilterData = useMemo(() => {
    let computedData = readyForDelivery;
    if (search) {
  ...
  }, [search, itemOffset, endOffset, readyForDelivery]);
...
export default connect(mapStateToProps, {})(withRouter(RadyDeliverdOrdersList));
