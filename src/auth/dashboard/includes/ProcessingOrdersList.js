...
const ProcessingOrdersList = (props) => {
  const { auth } = props;
  ...
  const pageCount = Math.ceil(processingOrders.length / itemsPerPage);

  // Invoke when user click to request another page.
  const handlePaginationClick = (event) => {
    const newOffset = (event.selected * itemsPerPage) % processingOrders.length;
  ...
  const allFilterData = useMemo(() => {
    let computedData = processingOrders;
    if (search) {
  ...
  }, [search, itemOffset, endOffset, processingOrders]);
        ...
        <div className='card-body border p-4'>
          <div>
            ...
            {!loading && allFilterData.length > 0 ? (
              ...
              <p className='text-center bold'> No Order found!</p>
            )}
          </div>
...
export default connect(mapStateToProps, {})(withRouter(ProcessingOrdersList));
