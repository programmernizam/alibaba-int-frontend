...
const RefundedOrders = (props) => {
  ...
  const pageCount = Math.ceil(refundedOrders.length / itemsPerPage);

  // Invoke when user click to request another page.
  const handlePaginationClick = (event) => {
    const newOffset = (event.selected * itemsPerPage) % refundedOrders.length;
  ...
  const allFilterData = useMemo(() => {
    let computedData = refundedOrders;
    if (search) {
  ...
  }, [search, itemOffset, endOffset, refundedOrders]);
...
export default connect(mapStateToProps, {})(withRouter(RefundedOrders));
